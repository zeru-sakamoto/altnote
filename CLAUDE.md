...
- `src/theme/store.ts` — resolves the active theme (by id, from `presetThemes` or the settings store's cached custom themes) and converts it via `src/theme/convert.ts`. `getActiveTheme()` is memoized by raw-theme object identity — callers (Editor's settings subscription, App's CSS-var effect) re-invoke it on _every_ settings change, not just theme ones, so this cache is load-bearing for avoiding redundant `HighlightStyle` rebuilds.
