...
describe('parseVscodeThemesUrl', () => {
  it('parses a full vscodethemes.com theme URL', () => {
    expect(parseVscodeThemesUrl('https://vscodethemes.com/e/dracula-theme.theme-dracula/dracula-theme')).toEqual({
  ...
  it('parses a bare path without protocol/host', () => {
    expect(parseVscodeThemesUrl('/e/enkia.tokyo-night/tokyo-night-storm')).toEqual({
      publisher: 'enkia',
  ...
  it('rejects URLs from a different host', () => {
    expect(parseVscodeThemesUrl('https://example.com/e/pub.ext/slug')).toBeNull();
  });

  it('rejects a vscodethemes.com URL that is not a theme page', () => {
    expect(parseVscodeThemesUrl('https://vscodethemes.com/')).toBeNull();
    expect(parseVscodeThemesUrl('https://vscodethemes.com/e/pub-no-extension')).toBeNull();
  });
